...
build_app_env=$build_dir/app.env.deploy.$build

sed -e "s/<build>/$build/g" $deploy_dir/docker-compose.deploy.template.yml >> $build_docker_compose
sed -e "s/<build>/$build/g" $deploy_dir/app.env.deploy.template >> $build_app_env


sed -e 's#<app_dir>#'$app_dir'#g' $build_docker_compose > $build_docker_compose.temp
...
c_php_fpm_build=astromancy.php-fpm.$build

docker  exec -it -u root $c_nginx_build chown -R www-data:www-data /var/log/nginx
docker  exec -it -u root $c_php_fpm_build chown -R www-data:www-data /var/www/astromancy
docker  exec -it -u root $c_php_fpm_build mkdir -p /var/log/astromancy
docker  exec -it -u root $c_php_fpm_build chown -R www-data:www-data /var/log/astromancy
